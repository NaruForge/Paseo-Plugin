...
이 문서는 **Paseo `0.8.0-beta.1`**가 Plugin에 열어 둔 UI 위치를 다룬다. 현재 저장소 소스는 아직 0.7.2 대상이며, 아래 내용은 [0.8 이관](../MIGRATION_0.8.md) 이후 사용할 계약이다. Plugin UI는 React Native component이며 desktop, browser, iOS, Android에서 같은 기여 계약을 사용한다.
