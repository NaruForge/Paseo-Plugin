...
Use [configuration guidance](docs/CONFIGURATION.md) for prerequisites. The commands above use source compatible with the target daemon; both plugin sources target 0.8.0-beta.1. Match each test daemon and app to the selected plugin/ref. For a remote 0.8 daemon, use the global form `paseo --host <target> plugin ls`. Verify both daemon and app versions. Remove temporary installations after verification; built-in 0.8 settings are deleted when their installation is removed.
