...
**Release: [v0.1.0-rc.3](https://github.com/NaruForge/Paseo-Plugin/releases/tag/v0.1.0-rc.3), targeting Paseo 0.8.0.** All four plugins use the exact final SDK and its revised Composer pill API. The collection remains a prerelease; see the [final-version evidence and limitations](docs/verification/paseo-0.8.0-release.md). Use final 0.8.0 daemon, app and CLI, not beta.1. The old [v0.1.0-rc.2](https://github.com/NaruForge/Paseo-Plugin/releases/tag/v0.1.0-rc.2) tag remains available for Paseo 0.7.2.

Provider Usage uses Paseo’s official usage API. Branch Garden remains preview; the other plugins remain experimental. Prior [user runtime](docs/verification/paseo-0.8-runtime.md) and [Git-source](docs/verification/paseo-0.8-git-source.md) reports retain their original scope and do not certify the final-version pill changes.

## Plugins

| Plugin | What it does | Requirements | Maturity |
| --- | --- | --- | --- |
| [`branch-garden`](plugins/branch-garden/) | Inspect registered Git projects, workspaces, branches and worktrees without changing Git state. | Current source: Paseo 0.8.0; Git; registered projects or workspaces | Preview; v0.1.0-rc.3 |
| [`command-deck`](plugins/command-deck/) | Save Project commands and run, inspect and stop PowerShell terminals from the Composer. | Windows Host, PowerShell 7, Paseo 0.8.0 | Experimental, v0.1.0-rc.3; app runtime verification pending |
...
1. Use final **Paseo 0.8.0 daemon, app and CLI**. Install the tools listed in the plugin guide on the daemon host.
...
Expect the chosen runtime to be `running` without load errors. The manifest ID is the default runtime ID; use the actual ID in later commands if installed with `--id`. Installations are per daemon. Remote 0.8 commands use `paseo --host <host> plugin ls`. See the plugin guides above for actions and [configuration](docs/CONFIGURATION.md#update-and-remove) for maintenance.
...
The v0.1.0-rc.3 collection includes all four plugins. Match the daemon, app and CLI to **0.8.0** and read the [final-version evidence](docs/verification/paseo-0.8.0-release.md). Git add/update/recovery for the three existing plugins is recorded in the [Git source verification](docs/verification/paseo-0.8-git-source.md); Command Deck Git activation remains unverified.
...
For the 0.8.0 release, review and pin `v0.1.0-rc.3`. Do not use `v0.1.0-rc.2` for 0.8 source; that tag is the 0.7.2 release and does not include Prompt Palette or Command Deck. Choose **one** command:
...
Upstream: [Plugin versions](https://paseo.sh/docs/plugins) · [v0.7 reference for the published release](https://paseo.sh/docs/plugins/v0.7/reference) · [v0.8 reference](https://paseo.sh/docs/plugins/v0.8/reference) · [Migration](https://paseo.sh/docs/plugins/v0.8/migration) · [Community projects](https://paseo.sh/docs/community)
