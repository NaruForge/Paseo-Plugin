...
if (server.name !== "@getpaseo/server" || server.version !== plugin.devDependencies?.["@getpaseo/plugin"] || server.version !== "0.8.0-beta.1") {
  throw new Error("Supply @getpaseo/server 0.8.0-beta.1 matching the plugin's exact SDK. Recheck the private compiler API before changing versions.");
}
