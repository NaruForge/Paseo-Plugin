...
Windows daemon에 명시적으로 설정한 허용 폴더를 탐색하고, 일반 파일과 선택한 파일·폴더를 같은 Tailnet의 PC 또는 모바일로 내려받는 Paseo 플러그인이다.

## 설정

기본으로 공개되는 폴더는 없습니다. daemon 사용자의 `~/.config/paseo-plugins/file-browser.json`에 허용 루트를 지정하세요. 기존 `C:\Projects` 사용자는 해당 경로도 명시적으로 설정해야 합니다. [설정과 이전 절차](../../docs/CONFIGURATION.md)를 따르세요. 최초 정상 로드 후 설정 변경은 plugin reload로 반영되며 기존 다운로드 URL이 만료됩니다.
...
현재 폴더 ZIP은 허용된 루트 전체에서 허용하지 않지만, 루트에 보이는 개별 프로젝트 폴더를 선택해 내려받을 수 있다. ZIP은 최대 10,000개 항목, 비압축 합계 2 GiB, 선택 폴더부터 깊이 64로 제한하며 한 번에 하나만 생성한다. 포함 후보에 민감 파일, link·junction 또는 지원하지 않는 항목이 있으면 전체 요청을 거부한다. ZIP 안에는 선택한 폴더 자체가 최상위 디렉터리로 포함된다.
