...
import type { ProviderUsage, UsageWindow } from "./provider-usage";
...
describe("provider usage logic", () => {
  it("parses agent provider ids without a fixed provider allowlist", () => {
    ...
    expect(parseAgentProviderId("customProvider/model")).toBe("customProvider");
  });
